...
THE SITE IS MULTILINGUAL (English, Spanish, Portuguese). Three rules keep this
script from damaging it -- before 2026-09-11 it followed none of them, and a
single run would have deleted the language alternates from every English page,
overwritten the Spanish and Portuguese heads with English breadcrumbs and
og:locale, replaced their bylines with English dates, filled the English hub
with all three languages, and dropped the localised top-level pages from the
sitemap.

  1. It writes ENGLISH pages only, and refuses any page whose <html lang> is
     not "en". Spanish and Portuguese heads belong to tools/i18n's builders.
  2. The hreflang alternates live inside the SEO block, so every block it
     writes carries them, generated by tools/i18n/langlinks.py from
     languages.json -- the same code that maintains them everywhere else.
  3. The English hub, its JSON-LD and its fallback list are English rows only;
     the sitemap lists every page in every language.

    python3 tools/seo.py --check    # what would change; writes nothing

--check must report only the changes you intend. Run it first, every time.
"""
import ast, json, os, re, html, datetime, sys
...
OVERLAY = {}        # --check: every write lands here instead of on disk
...
_CFG, _SLUGS = langlinks.load()
_EN_SLUGS = sorted(f[:-5] for f in os.listdir(os.path.join(ROOT, "articles")) if f.endswith(".html"))
GROUPS = langlinks.groups(_CFG, _SLUGS, _EN_SLUGS)


def alternates(rel):
    """The hreflang block for a page, from the manifest -- or "" if the page
    has no translations."""
    members = GROUPS.get(rel)
    return langlinks.alt_block(_CFG, members) if members else ""


def assert_english(rel, s):
    lang = tag(s, r'<html lang="([^"]*)"')
    assert lang == "en", "%s is <html lang=\"%s\">: seo.py writes English pages only" % (rel, lang)
...
TOP = [
    ...
    dict(file="resources.html", slug="resources", url=SITE + "/resources.html",
         ...
         alt="Advertising, inventory, cash and margin connected in one view for an Amazon seller"),
    ...
    dict(file="contact.html",   slug="contact",   url=SITE + "/contact.html",
         ...
         alt="Seller messages arriving, read, and turning into changes in the product"),
...
def head_block(url, og_title, description, image, alt, og_type, jsonld, rel=None):
    alt_links = alternates(rel) if rel else ""
    L = [START,
         '<link rel="canonical" href="%s">' % url] + ([alt_links] if alt_links else []) + [
...
def do_articles(resources):
    n = 0
    for r in [r for r in resources if r.get("language", "en") == "en"]:
        ...
        apply_head(rel, head_block(url, og_title, og_desc, img, alt, "article", graph, rel))
...
def do_top(resources):
    for p in TOP:
        ...
        if p["slug"] == "home":
            ...
            nodes = [ORG, WEBSITE, {
                ...
                "hasPart": [{"@type": "Article",
                             ...
                             "url": "%s/%s" % (SITE, r["url"]),
                             "datePublished": r["published"]}
                            for r in sorted(english(resources), key=lambda x: x["published"],
                                            reverse=True)],
                "inLanguage": "en",
            }, breadcrumbs(trail)]
        else:
        ...
        apply_head(p["file"], head_block(
            ...
            ld({"@context": "https://schema.org", "@graph": nodes}), p["file"]))
    return len(TOP)


def english(resources):
    return [r for r in resources if r.get("language", "en") == "en"]
...
def do_hub(resources):
    ...
    rows = []
    for r in sorted(english(resources), key=lambda x: x["published"], reverse=True):
        ...
        rows.append(
            ...
            '  </a>'
            % (r["url"], ("    %s\n" % thumb) if thumb else "", esc(r["title"]), esc(r.get("summary", "")),
...
def main():
    ...
    do_robots()
    print("head blocks: %d top-level + %d articles (English)" % (t, a))
    ...
    for p in sorted(OVERLAY):
        disk = open(os.path.join(ROOT, p), encoding="utf-8", newline="").read()
        if OVERLAY[p] == disk:
            continue
        if p == "sitemap.xml":
            a_ = set(re.findall(r"<url>\s*<loc>(.*?)</loc>\s*<lastmod>(.*?)</lastmod>", disk))
            b_ = set(re.findall(r"<url>\s*<loc>(.*?)</loc>\s*<lastmod>(.*?)</lastmod>", OVERLAY[p]))
            if a_ == b_:
                print("  sitemap.xml: same %d URLs and dates, order only" % len(a_))
                continue
            print("  sitemap.xml: would ADD %s / DROP %s" % (sorted(b_ - a_)[:4], sorted(a_ - b_)[:4]))
        changed.append(p)
    same = len(OVERLAY) - len(changed)
    print("\n--check: %d files unchanged, %d would change:" % (same, len(changed)))
    for p in changed:
        print("      " + p)
