title:        Should You Start Advertising on Day One or Wait for Reviews?
...
- Why experienced sellers reach opposite conclusions
- The case for starting on day one
- The case for waiting for reviews
- How to decide which cost is larger
- What one budget buys at two conversion rates
- Whether you can start small instead
- How stock and price change the answer

## Why do experienced sellers disagree about when to start?
...
## What is the case for starting on day one?
...
## What is the case for waiting until the listing has reviews?
...
## How do you decide which cost is larger?
...
## What does the same budget buy at two different conversion rates?
...
Average CPC | $1.00
Clicks bought, $300.00 divided by $1.00 | 300
Ad conversion rate | 3%
Ad-attributed orders, 3% of 300 | 9
Selling price | $30.00
Ad-attributed sales, 9 times $30.00 | $270.00
ACOS, $300.00 divided by $270.00 | 111.1%
...
Average CPC | $1.00
Clicks bought, $300.00 divided by $1.00 | 300
Ad conversion rate | 9%
Ad-attributed orders, 9% of 300 | 27
Selling price | $30.00
Ad-attributed sales, 27 times $30.00 | $810.00
ACOS, $300.00 divided by $810.00 | 37.0%
Profit per unit before advertising | $9.00
...
[[FIGURE: Two launch paths: advertising from day one, or waiting until reviews arrive]]

## Can you start small instead of choosing?
...
[[What to do]]
1. Pick a daily budget you would not mind losing entirely for two to three weeks, and write the total down first.
2. Run one exact-match campaign on a handful of precisely relevant keywords, and one targeting competitor product pages.
3. Confirm impressions are arriving. Zero impressions is not a conversion problem, and waiting does not fix it.
4. Once a campaign has a few hundred clicks, calculate ad conversion rate in Campaign Manager: ad-attributed orders divided by clicks.
5. Scale only after that number stops getting worse, and only if your stock can cover the higher sales rate.
...
## How do stock and launch price change the answer?
...
Ad conversion rate, used to judge whether early clicks are converting — Campaign Manager > Clicks and Ad-attributed orders columns for the ASIN or campaign, divided — covers ad traffic only, attributed within Amazon's attribution window, for the selected date range — matches the claim: yes.

Average CPC, used in the worked example — Campaign Manager > CPC column for the campaign and date range — covers ad clicks only in that range — matches the claim: yes.

Units available to sell, used for the days-of-supply test — Seller Central > Inventory > Manage FBA Inventory, "Available" column for the SKU — covers units at Amazon fulfilment centres sellable now for that single SKU; excludes inbound and reserved units — matches the claim: yes, provided the reader uses one SKU and does not add inbound units.

Profit per unit before advertising, used in the worked example — the reader's own unit and freight costs combined with Amazon's fee preview in Seller Central — covers one unit of one SKU, before ad cost and before returns — matches the claim: uncertain, because the name of Amazon's fee estimation tool in Seller Central has changed more than once; the article therefore describes the figure and does not name a screen.

Early visibility for new listings — no Amazon report, screen or help page documents this; it is reported by sellers and inferred from observed search behaviour — covers nothing the reader can open and check — matches the claim: uncertain. Independent write-ups agree it is not officially confirmed and disagree on duration, commonly stated as 15 to 30 days, with other accounts describing a longer or differently shaped ramp. CONFIRMED 2026-09-12 by Leon: the honeymoon period is real, but nobody knows with certainty how long it lasts. That is what the article already says -- sellers report it, Amazon does not document it, and no duration should be planned around. The wording stands unchanged; what is resolved is the flag, not the caveat.

Vine enrolment fee and unit cap per parent ASIN — Seller Central > Vine enrolment page — covers one parent ASIN in one marketplace — matches the claim: yes, CONFIRMED 2026-09-12 against Leon's own Vine enrolment page. The tiers are 1 to 2 units free, 3 to 10 units $75, and 11 to 30 units $200, which also gives the unit cap per parent ASIN as 30. The article now states them, scoped to the time of writing, and still sends the reader to their own enrolment page because the tiers differ by marketplace.

Review request Amazon sends on the seller's behalf — Seller Central > Orders > order detail page, "Request a review" — covers one delivered order, one request only, inside a limited window after delivery — matches the claim: uncertain on the exact length of that window, which sources state differently. The article states no window and says only that the request requires an order to exist.

## New terms

- early visibility window — the extra visibility sellers report a brand-new listing receives in its first weeks. Everyday phrase: the head start a new listing seems to get. Translate as an observation, never as the name of an Amazon programme; the surrounding sentences deny that it is documented, and that denial must survive translation.
- ad conversion rate — ad-attributed orders divided by clicks, from Campaign Manager. Already defined in this set; listed because this article leans on it as the main decision signal.
- ad-attributed order — an order Amazon credits to an ad click within the attribution window. Already defined in this set; used here in the worked example.
- unit session percentage — the Seller Central Business Reports column dividing units ordered by all listing sessions. Everyday phrase: the share of listing visits that ended in an order. Appears only as the number readers are told not to use here; the contrast with ad conversion rate must survive translation.
- profit per unit before advertising — what one sale leaves after Amazon's fees and the cost of getting the unit to Amazon, before any ad cost. Everyday phrase: what a sale is worth to you before you pay for the click. Kept plain instead of "contribution margin", an accounting term beginners do not share.
...
- indexed — Amazon has associated the listing with a search term, so it can appear in results for it. Everyday phrase: the listing can be found for that search.
- No new callout labels were needed. Labels used: The asymmetry, The limit, The trap, The premise, The rule, Worked example, What to do, The distinction, The test, The mismatch.
